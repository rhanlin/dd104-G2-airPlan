...
try {
	...
	$sql = "select  `letterLike`.letLikeTime, `letter`.letTitle,`letterLike`.memNo, `member`.memName  from `letter` left outer join `letterLike` on letter.letNo = letterLike.letNo left outer join `member` on letterLike.memNo = member.memNo where letter.memNo=:memNo order by letLikeTime desc";
